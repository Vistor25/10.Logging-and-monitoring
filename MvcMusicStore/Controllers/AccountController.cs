    ...
    public class AccountController : Controller
    {
        ...
        private async Task MigrateShoppingCart(string userName)
        {
            using (var storeContext = new MusicStoreEntities())
            {
                ...
                logger.Info($"Card was migrated for user {userName} ");
        ...
        public async Task<ActionResult> Manage(ManageUserViewModel model)
        {
            ...
            if (hasPassword)
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (result.Succeeded)
                    {
                        logger.Info($"User with id {User.Identity.GetUserId()} successfully changed password");
                        return RedirectToAction("Manage", new { Message = ManageMessageId.ChangePasswordSuccess });
                    }

                    logger.Error($"User with id {User.Identity.GetUserId()} cannot change password");
                    AddErrors(result);
                }
            }
            else
